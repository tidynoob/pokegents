...
func TestStateMachine(t *testing.T) {
	tests := []struct {
		...
		{"done to busy", "done", "UserPromptSubmit", "", "busy", false},
		...
		// Notification(idle_prompt) — only busy → done
		{"busy to done on idle_prompt", "busy", "Notification", "idle_prompt", "done", false},
		{"done unchanged on idle_prompt", "done", "Notification", "idle_prompt", "done", false},
...
// after Stop don't overwrite "done" with "busy".
func TestPostToolUseRaceGuard(t *testing.T) {
	...
	// Late PostToolUse arrives — must NOT overwrite done
	...
	if sf, ok := sm.statuses[sid]; ok {
		if sf.State != "done" {
			t.Errorf("state should still be done, got %s", sf.State)
		}
	}
}
